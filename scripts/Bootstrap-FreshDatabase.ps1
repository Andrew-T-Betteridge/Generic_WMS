...
if ($Target -eq "PROD") {
    ...
    $tag = (& git -C $repoRoot tag --points-at HEAD "dynetic-wms-v0.3.7").Trim()
    if ($tag -ne "dynetic-wms-v0.3.7") {
        Stop-Bootstrap "PROD requires HEAD tagged exactly dynetic-wms-v0.3.7."
    }
}
