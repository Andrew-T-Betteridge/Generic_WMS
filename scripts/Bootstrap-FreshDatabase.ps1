...
if ($Target -eq "PROD") {
    ...
    $tag = (& git -C $repoRoot tag --points-at HEAD "dynetic-wms-v0.3.5").Trim()
    if ($tag -ne "dynetic-wms-v0.3.5") {
        Stop-Bootstrap "PROD requires HEAD tagged exactly dynetic-wms-v0.3.5."
    }
}
